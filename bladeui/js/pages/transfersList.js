...
class Transfers extends BaseClass
{
  ...
  renderTransfersList(info)
  {
    ...
    for (let i = 0; i < info.length; i += 1)
    {
      ...
      newRow.innerHTML = `${transferStatus}<p class="date">${date}</p><p class="quantity">${rewardInfo}</p><p class="unit">ADB</p><p class="txhash"><a href="https://rinkeby.etherscan.io/tx/${info[i].txhash}">${info[i].txhash}.</button></p>`;
