...
let bladeUserId = "";
let token = "";
updateAdReplacementInfo();
function updateAdReplacementInfo()
{
  browser.storage.sync.get(null, (data) =>
  {
    if (data)
    {
      reviveUrl = data.bladeAdserverUrl;
      bladeUserId = data.bladeUserData ? data.bladeUserData.userCode : "";
      token = data.bladeUserData ? data.bladeUserData.token : "";
    }
  });
}
...
async function getToken()
{
  return new Promise(resolve =>
  {
    browser.storage.sync.get(null, (data) =>
    {
      if (data)
      {
        resolve(data.bladeUserData.token);
      }
    });
  });
}

async function proccessUserToken()
{
...
port.on("background.getUserToken", proccessUserToken);
port.on("background.getAdReplacementInfo", (message, sender) =>
{
  let docDomain = extractHostFromFrame(sender.frame);
  const selectors = ElemHide.getSelectorsForDomain(docDomain);

  if (!bladeUserId || reviveUrl)
  {
    updateAdReplacementInfo();
  }
